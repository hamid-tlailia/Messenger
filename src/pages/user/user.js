...
const User = () => {
  ...
  return (
    ...
    <div className="profile-content">
      <h5 className="fw-bold"><i class="fas fa-qrcode text-primary"></i> Profile</h5>
...
<img src= {userThree} className='profile-avatar shadow-4-strong p-1 me-1' alt="chat" />
<span className='text-primary p-2 shadow-2' onClick={() => {
...
<div className="d-flex flex-column flex-lg-row flex-md-row justify-content-center align-items-start gap-2 change-image">
  <label htmlFor="file" className='btn btn-light text-dark remove-select'>Select new photo</label>
  <button className='btn btn-danger text-light remove-select'>Remove photo</button>
...
<form className="d-flex flex-column">
  ...
  <input type="text" className= {updateName ? "name-input inputs update" : "name-input inputs"} id='name' placeholder='Hamidos'/>
  <i className="far fa-pen-to-square" onClick={(e) => {
    setUpdateName(!updateName)
    !updateName && e.target.parentNode.querySelector("#name").focus()
  }}></i>
  {
    updateName && <i className="far fa-circle-check fs-4 text-success" ></i> 
  }
  </div>
  <label className='label'>Email :</label>
  <div className="d-flex flex-row gap-2 justify-content-center align-items-center update-area">
  <input type="email"  id="email" className= {updateEmail ? "email-input inputs update" : "email-input inputs"} placeholder='tlailia757@gmail.com'/>
  <i className="far fa-pen-to-square" onClick={(e) => {
    setUpdateEmail(!updateEmail)
  !updateEmail &&  e.target.parentNode.querySelector("#email").focus()
  }}></i>
  {
    updateEmail && <i className="far fa-circle-check fs-4 text-success" ></i> 
  }
  </div>
</form>
...
<div className="password-update-area">
  ...
  <p>ensure your account is using a long rondom password to stay secure</p>
  <form className="d-flex flex-column flex-lg-row flex-md-row justify-content-start gap-3 align-items-start">
    <input type="text" className='password-input' placeholder='Old password'/>
    <input type="text" className='password-input' placeholder='New password'/>
    <button className='btn btn-primary'>save</button>
  </form>
</div>
{/* friends list area start */}
<div className= {showModal ? "friends-list-parent show" : "friends-list-parent"}>
  <div className="d-flex flex-row justify-content-between align-items-center">
    <span className='text-primary'><i class="fas fa-user-plus"></i> Friends list</span>
    <i class="fas fa-xmark close-modal" onClick={() => {
  setShowModal(!showModal)
}}></i>
  </div>
  <hr />
  ...
  <div className="d-flex flex-row justify-content-center align-items-center gap-2">
    <button className="lists-nav-btns w-50" onClick={() => {
...
{/* user friends list */}
<div className= {showRequestList ? "user-friends-list" : "user-friends-list active"}>
  ...
  <div className="chat mb-2 gap-auto" style={{width:"100%"}}>
              ...
              <div className="chat-details w-100 text-start">
                ...
                <p><i class="fas fa-check text-success icon-user-list"></i> friend</p>
              </div>
...
{/* user request list */}
<div className= {showRequestList ? "user-requests-list active" : "user-requests-list"}>
<div className="answer-friends-request-area">
  <p className='fw-bold mb-2'>Friends requests :</p>
{/* request 1 */}
<div className="friends-section mb-2" >
              <img
                src={userThree}
                className="chat-avatar shadow-4-strong p-1"
                alt="chat"
              />
              <div className="chat-details w-25 text-start">
                  <strong className='mt-2'>Hiba</strong>
                <p className='row-center'><i class="fas fa-clock"></i> waiting</p>
              </div>
              <div className="d-flex flex-row justify-content-start align-items-start gap-2">
                <button className="btn btn-success"><i class="fas fa-circle-check text-light icon-user-list"></i></button>
                <button className="btn btn-danger"><i class="far fa-trash-can text-light icon-user-list"></i></button>
              </div>
            </div>
            {/* request 2 */}
...
<div className="friends-section mb-2" >
              ...
              <div className="chat-details w-100 text-start">
                  <strong className='mt-2'>Khaled</strong>
                <p><i class="fas fa-user-plus text-primary"></i> user</p>
              </div>
                <button className="btn btn-primary d-flex flex-row gap-1 justify-content-center align-items-center">
                  <i class="fas fa-user-plus text-light"></i> Add</button>
            </div>
</div>
</div>
</div>
  ...
  <div class="sign"><svg viewBox="0 0 512 512">
    <path d="M377.9 105.9L500.7 228.7c7.2 7.2 11.3 17.1 11.3 27.3s-4.1 20.1-11.3 27.3L377.9
     ...
     256c0 17.7 14.3 32 32 32l64 0c17.7 0 32 14.3 32 32s-14.3 32-32 32l-64 0c-53 0-96-43-96-96L0 128C0 75 43 32 96 32l64
      0c17.7 0 32 
    14.3 32 32s-14.3 32-32 32z"></path></svg></div>
